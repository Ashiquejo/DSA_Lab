...
void firstFit(Block *head, int processSize[], int n) {
    int allocation[n];
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (curr) {
            if (curr->size >= processSize[i]) {
                allocation[i] = curr->id;
                curr->size -= processSize[i];
                break;
            }
            curr = curr->next;
        }
    ...
    Block *curr = head;
    int totalRemainingSize = 0;

    while (curr) {
        printf("Block %d -> %d\n", curr->id, curr->size);
        totalRemainingSize += curr->size;
        curr = curr->next;
    }
}
